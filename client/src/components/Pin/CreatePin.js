...
const CreatePin = ({ classes }) => {
  ...
  const handleImageUpload = async () => {
    ...
    data.append("cloud_name", "djjf5jpbw");
    const response = await axios.post("https://api.cloudinary.com/v1_1/djjf5jpbw/image/upload", data);
  ...
  return <form className={classes.form}>
    ...
    <div className={classes.contentField}>
      <TextField name="content" label="Content" multiline rows="6" margin="normal" fullWidth variant="outlined" 
        onChange={e => setContent(e.target.value)} rows={ mobileSize ? "3" : "6" }
      />
    </div>
    <div>
